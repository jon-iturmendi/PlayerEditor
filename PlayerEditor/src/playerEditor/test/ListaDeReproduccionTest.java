...
import playerEditor.ListaDeReproduccion;
...
public class ListaDeReproduccionTest {
	...
	// Chequeo de error por getFic(�ndice) por debajo de 0
	@Test(expected = IndexOutOfBoundsException.class)  
	public void testGet_Exc2() {  
		lr2.getFic(-1);  // Debe dar error porque a�n no existe la posici�n -1
	} 

	// Chequeo de funcionamiento correcto de getFic(�ndice)
	@Test
	...
	// Chequeo de a�adido y borrado de elementos
	@Test
	public void addremove() {
		File nulo = null;
		lr3.add( nulo );  // A�ado null en posici�n 5 (6� elemento)
	...
	// Chequeo de m�todos de seleccion
	@Test public void irAPrimero() {
		...
		lr2.setFicErroneo( 0, true );
			lr2.irAPrimero();
			assertEquals( "En lr2 con fich. err�neo no se puede seleccionar ninguno", -1, lr2.getFicSeleccionado() );
			lr2.setFicErroneo( 0, false );
		lr3.irAPrimero();
			assertEquals( "Selecci�n correcta en lr3", 0, lr3.getFicSeleccionado() );
	}
	...
	@Test public void irAUltimo() {
		...
		lr2.setFicErroneo( 0, true );
			...
			lr2.setFicErroneo( 0, false );
		lr3.irAUltimo();
			assertEquals( "Selecci�n correcta en lr3", lr3.size()-1, lr3.getFicSeleccionado() );
	}

	@Test public void irAAnterior() {
		assertFalse( "En lr1 no se puede seleccionar ninguno", lr1.irAAnterior() );
		lr2.setFicErroneo( 0, true );
			assertFalse( "En lr2 con fich. err�neo no se puede seleccionar ninguno", lr2.irAAnterior() );
			lr2.setFicErroneo( 0, false );
		lr3.irAUltimo();
			assertTrue( "Anterior correcto en lr3", lr3.irAAnterior() );
			assertEquals( "Selecci�n correcta en lr3", lr3.size()-2, lr3.getFicSeleccionado() );
			lr3.irAPrimero();
	...
	@Test public void irASiguiente() {
		...
		lr2.setFicErroneo( 0, true );
			assertFalse( "En lr2 con fich. err�neo no se puede seleccionar ninguno", lr2.irASiguiente() );
			lr2.setFicErroneo( 0, false );
		lr3.irAPrimero();
			assertTrue( "Siguiente correcto en lr3", lr3.irASiguiente() );
			assertEquals( "Selecci�n correcta en lr3", 1, lr3.getFicSeleccionado() );
			lr3.irAUltimo();
			assertFalse( "No hay siguiente al �ltimo en lr3", lr3.irASiguiente() );
	}
	
	@Test public void irA() {
		assertFalse( "En lr1 no se puede seleccionar ninguno", lr1.irA(0) );
		lr2.setFicErroneo( 0, true );
			assertFalse( "En lr2 con fich. err�neo no se puede seleccionar ninguno", lr2.irA(0) );
			lr2.setFicErroneo( 0, false );
		assertTrue( "El 0 correcto en lr3", lr3.irA(0) );
			assertEquals( "Selecci�n correcta en lr3", 0, lr3.getFicSeleccionado() );
		assertTrue( "El 3 correcto en lr3", lr3.irA(3) );
			assertEquals( "Selecci�n correcta en lr3", 3, lr3.getFicSeleccionado() );
	...
	@Test public void irARandom() {
		...
		for (int i=0; i<5; i++) {
			...
			assertFalse( "En lr2 con fich. err�neo no se puede seleccionar ninguno", lr2.irARandom() );
			assertTrue( "Sel. aleatoria correcta en lr3", lr3.irARandom() );
			assertTrue( "Selecci�n correcta en lr3", lr3.getFicSeleccionado()>=0 && 
